import { query } from "../../rag-api/src/db.js";
...
type PrevBlockRow = { id: string; path: string; content_hash: string };
...
export async function processDoc(job: any) {
  ...
  const prev = await query<PrevBlockRow>(
  ...
  try {
    for (const b of blocks) {
      const existed = prevByPath.get(b.path) as PrevBlockRow | undefined;
    ...
    for (const c of changed) {
      await query(`delete from chunks where block_id = $1`, [c.block_id]);
    ...
    await query(`update documents set status='needs_index', updated_at=now() where id=$1`, [
      document_id,
    ]);

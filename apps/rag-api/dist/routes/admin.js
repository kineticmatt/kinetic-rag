...
async function adminRoutes(app) {
    ...
    app.post("/admin/tenant", async (req, reply) => {
        ...
        if (!slug || !name) {
            return reply
                .code(400)
                .send({ error: { code: "BAD_INPUT", message: "slug and name required" } });
        }
        const r = await (0, db_js_1.query)(`insert into tenants (slug, name)
       values ($1, $2)
    ...
    app.post("/admin/keys", async (req, reply) => {
        ...
        if (!tenant_id || !name) {
            return reply
                .code(400)
                .send({ error: { code: "BAD_INPUT", message: "tenant_id and name required" } });
        }
        ...
        const r = await (0, db_js_1.query)(`insert into api_keys (id, tenant_id, name, key_hash)
       values ($1, $2, $3, $4)
    ...
    app.get("/admin/keys", async (req, reply) => {
        ...
        const r = await (0, db_js_1.query)(`select id, tenant_id, name, created_at, revoked_at
         from api_keys
    ...
    app.post("/admin/keys/revoke", async (req, reply) => {
        ...
        if (!id) {
            return reply
                .code(400)
                .send({ error: { code: "BAD_INPUT", message: "id required" } });
        }
        await (0, db_js_1.query)(`update api_keys set revoked_at = now() where id = $1`, [id]);
        reply.send({ ok: true });
    });
}

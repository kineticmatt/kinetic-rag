...
async function documentsRoutes(app) {
    ...
    app.post("/documents", async (req, reply) => {
        ...
        // Upsert document row
        const tenantId = req.tenantId; // set by authGuard if you wire it here
        const r = await db_js_1.pool.query(`insert into documents (tenant_id, uri, title, format, doc_type, allow_groups, status, updated_at)
